...
function wsm_dismiss_notice()
	...
	//	update_option(WSM_PREFIX.'ReportEmails',sanitize_text_field($_POST[WSM_PREFIX.'ReportEmails']));
		...
		if(is_admin())
		{
			...
			if(!empty($_POST[WSM_PREFIX.'UserRolesExcluding']))
			{
				$wsmUserRoles = '';
				foreach ($_POST[WSM_PREFIX.'UserRolesExcluding'] as $v)
				{
					$wsmUserRoles .= $v.",";
				}
				
			$wsmUserRoles = substr($wsmUserRoles,0,-1);
			@update_option(WSM_PREFIX.'UserRolesExcluding',$wsmUserRoles);
			}else{
			@update_option(WSM_PREFIX.'UserRolesExcluding','');	
			}
	...
	function wsmViewAddOns(){
		...
		if( is_array( $addons ) && count( $addons ) ){
			...
			foreach( $addons as $key => $addon ){
				...
				echo sprintf('<li><a class="%s" href="#%s">%s</a></li>', esc_attr($class), esc_url($key), esc_html($addon) );
				$class = '';
			}
			echo '</ul>';
			...
			foreach( $addons_settings as $key => $addons_setting ){
				...
				echo sprintf( '<table id="%s" style="display:%s" class="form-table">', esc_attr($key), esc_attr($visible) );
				$visible = '';
				$field_name = $key.'_enable';
				$checked = isset( $setting_values[$field_name] ) && $setting_values[$field_name] ? 'checked' : ''; 
				echo sprintf( '<tr><th>%s</th><td><label class="switch"><input name="%s" type="checkbox" %s value="1"><div class="slider round"></div></label></td></tr>', __( 'Enable','wp-stats-manager'), esc_attr($field_name), esc_attr($checked) );
				foreach( $addons_setting as $setting ){
					$field_name = $setting['id'];
					echo sprintf( '<tr><th>%s</th><td>', esc_html($setting['label']) );
					switch( $setting['type'] ){
						case 'checkbox';
							$checked = isset( $setting_values[$field_name] ) && $setting_values[$field_name] ? 'checked' : ''; 
							echo sprintf('<label class="switch"><input name="%s" type="checkbox" %s value="1"><div class="slider round"></div></label>', esc_attr($setting['id']), esc_attr($checked) );
							break;
						case 'select':
							echo sprintf( '<select name="%s">', esc_attr($setting['id']) );
							...
							if( isset( $setting['values'] ) ){
								foreach( $setting['values'] as $key => $value ){
									...
									echo sprintf( '<option %s value="%s">%s</option>', esc_attr($selected), esc_attr($key), esc_html($value) );	
								}		
							}
							echo '</select>';
							break;	
						case 'post_type':
							echo sprintf( '<select multiple="multilple" class="posttype_dropdown" name="%s[]">', esc_attr($setting['id']) );
							...
							$post_types = get_post_types($args, 'names');
								foreach( $post_types as $value ){
									if( $value != 'attachment' ){
										...
										echo sprintf( '<option %s value="%s">%s</option>', esc_attr($selected), esc_attr($value), esc_html($value) );	
									}
								}		
							echo '</select>';
							break;
    ...
    function wsmViewSettings(){
		...
		$UserRolesExcluding = get_option(WSM_PREFIX.'UserRolesExcluding');
		...
		foreach( $report_stats_list as $key => $value ){
			...
			if( !in_array($key, array( 'recent_visit_pages', 'traffic_by_title_stats', 'recent_active_visitors' ) ) ){
				$reportStatsHTML .= sprintf( '<tr><td>%s</td><td><label class="switch"><input name="%s" type="checkbox" %s value="%s" ><div class="slider round"></div></label></td></tr>', esc_html($value), WSM_PREFIX.'ReportStats[]', ($status?'checked':''), esc_attr($key) );
			}
			$reportWidget .= sprintf( '<div data-id="%s">%s</div>', esc_attr($key), esc_html($value) );
			$dashboardNormalWidget .= sprintf( '<li><i class="handle"></i> %s <label class="switch"><input name="%s" type="checkbox" %s value="%s" ><div class="slider round"></div></label></li>', esc_html($value), WSM_PREFIX.'Dashboard_widget[normal][]', ($status?'checked':''), esc_attr($key) );
			$pluginNormalWidget .= sprintf( '<li><i class="handle"></i> %s <label class="switch"><input name="%s" type="checkbox" %s value="%s" ><div class="slider round"></div></label></li>', esc_html($value), WSM_PREFIX.'Plugin_widget[normal][]', ($status?'checked':''), esc_attr($key) );
		}
		if( count( $dashboardNormalWidgetList ) ){
			...
			foreach( $dashboardNormalWidgetList as $widget ){
				...
				$dashboardNormalWidget .= sprintf( '<li><i class="handle"></i> %s <label class="switch"><input name="%s" type="checkbox" %s value="%s" ><div class="slider round"></div></label></li>', $report_stats_list[$widget], WSM_PREFIX.'Dashboard_widget[normal][]', ($status?'checked':''), esc_attr($widget) );
			}
		}
		if( count( $dashboardSideWidgetList ) ){
			foreach( $dashboardSideWidgetList as $widget ){
				...
				$dashboardSideWidget .= sprintf( '<li><i class="handle"></i> %s <label class="switch"><input name="%s" type="checkbox" %s value="%s" ><div class="slider round"></div></label></li>', $report_stats_list[$widget], WSM_PREFIX.'Dashboard_widget[side][]', ($status?'checked':''), esc_attr($widget) );
			}
		}
		if( count( $pluginNormalWidgetList ) ){
			...
			foreach( $pluginNormalWidgetList as $widget ){
				...
				//$pluginNormalWidget .= sprintf( '<li><i class="handle"></i> %s <label class="switch"><input name="%s" type="checkbox" %s value="%s" ><div class="slider round"></div></label></li>', $report_stats_list[$widget], WSM_PREFIX.'Plugin_widget[normal][]', ($status?'checked':''), esc_attr($widget) );
			}
		}
		if( count( $pluginSideWidgetList ) ){
			foreach( $pluginSideWidgetList as $widget ){
				...
				$pluginSideWidget .= sprintf( '<li><i class="handle"></i> %s <label class="switch"><input name="%s" type="checkbox" %s value="%s" ><div class="slider round"></div></label></li>', $report_stats_list[$widget], WSM_PREFIX.'Plugin_widget[side][]', ($status?'checked':''), esc_attr($widget) );
			}
		}
    ...
    foreach ($available_roles_names as $role_key => $role_name) { //we iterate all the names
       ...
       // print_r($array_of_capabilities);
		...
		$UserRolesExcluding = get_option(WSM_PREFIX.'UserRolesExcluding');
		$UserRoles_arr2 = explode(',',$UserRolesExcluding);
		...
		$html .= '<tr>
                    <th scope="row"><label for="'.WSM_PREFIX.'KeepData1">'.__('Excluding Roles from statistics','wp-stats-manager').'</label></th>
                    <td>';
		$html .="<select id='UserRolesExcluding' name='wsmUserRolesExcluding[]' multiple='true' style='width:50%;'>";

		foreach($available_roles_capable as $role=>$role_name)
		{
			$translated_role_name = $role_name;
			if(in_array($role, $UserRoles_arr2))
			{
...
$wsmRobots = get_option(WSM_PREFIX.'Robots');
		...
		foreach($wsm_robots as $k=>$v)
		{
			if(in_array($v, $wsmRobots_arr))
			{
			 ...
			 $html .="<option value='".esc_attr($v)."' ".esc_attr($selected_value) .">".esc_html($v)."</option>";
		}
		$html .='</select>';
                ...
                <tr>
                    ...
                    <td><textarea cols="50" id="'.WSM_PREFIX.'ReportEmails" name="'.WSM_PREFIX.'ReportEmails">'.esc_html($reportEmails).'</textarea>&nbsp;&nbsp;<a href="#" class="button button-primary send_test_mail">Send Test Mail</a>
						...
						<div id="dashboard-widgets-wrap" class="wsmMetaboxContainer">
							<div id="dashboard-widgets" class="metabox-holder">
								<form name="wsmmainMetboxForm" id="wsmmainMetboxForm" method="post">
									<input id="_wpnonce" name="_wpnonce" value="'.wp_create_nonce('my-nonce').'" type="hidden"><input type="hidden" name="_wp_http_referer" value="/wp-admin/admin.php?page=wsm_ipexc" />
        ...
        $wsmAdminJavaScript.='
            <script>jQuery("#'.WSM_PREFIX.'Country").on(\'change\', function() {
              ...
              jQuery.ajax({
                   ...
                   //jQuery("#'.WSM_PREFIX.'Country option[value=\'"+timezone+"\']").attr("selected", "selected");
				   jQuery(\'#submit\').prop(\'disabled\', false);
               });
            })
			new SlimSelect({
				  select: \'#UserRolesExcluding\'
				});
    ...
    function wsmShowMainPageLayout($page){
        ...
        echo '</form>';
        echo wsmInitPlugin::wsm_strip_tags($this->endMetaBoxWrapper);        
