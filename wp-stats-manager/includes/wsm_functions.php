...
function wsmValidateDateTime($dateStr, $format='Y-m-d'){ 
	...
	try{
		...
		echo sprintf( '<p>'. __( 'Message: This timezone is not supported in this PHP version %s.','wp-stats-manager') .'</p>', phpversion() );
	}
}
...
function wsmUrlToPostid( $url ) {
    ...
    if ( $post_id == 0 ) {
        ...
        foreach ( $cpts as $cpt_name => $cpt ) {
            if ( isset($cpt->rewrite['slug']) && !empty($cpt->rewrite['slug']) ) {
                $cpt_slug = $cpt->rewrite['slug'];
                if ( strlen( $path ) > strlen( $cpt_slug ) && substr( $path, 0, strlen( $cpt_slug ) ) == $cpt_slug ) {
                    $slug = substr( $path, strlen( $cpt_slug ) );
                    $query = new WP_Query( array(
...
function wsmFnGetLocationInfo($ip){        
    ...
    $infoResult_array = array();
	
	// first API check
    $infoURL='http://ip-api.com/json/';
    if($ip!=''){
        $infoURL.=$ip;
    }
    ...
    $infoResult=wsmFnSendCurlRequest($infoURL);
	if(isset($infoResult->city) && isset($infoResult->countryCode))
	   {

		   $infoResult_array['city'] = $infoResult->city;
		   $infoResult_array['country'] = $infoResult->country;
		   $infoResult_array['countryCode'] = $infoResult->countryCode;
		   $infoResult_array['regionName'] = $infoResult->regionName;
		   $infoResult_array['lat'] = $infoResult->lat;
		   $infoResult_array['lon'] = $infoResult->lon;

		   return $infoResult_array;
	   }
    
	/*
    // second check, if first failed
	$infoURL='https://geolocation-db.com/json/';
    if($ip!=''){
        $infoURL.=$ip;
    }
    ...
    $infoResult=wsmFnSendCurlRequest($infoURL);
        ...
        if(isset($infoResult->city) && isset($infoResult->country_code))
		   {
	
			   $infoResult_array['city'] = $infoResult->city;
			   $infoResult_array['country'] = $infoResult->country_name;
			   $infoResult_array['countryCode'] = $infoResult->country_code;
			   $infoResult_array['regionName'] = '';
			   $infoResult_array['lat'] = $infoResult->latitude;
			   $infoResult_array['lon'] = $infoResult->longitude;
	...
	*/
	...
	// last check, if second failed
	$infoURL='http://www.geoplugin.net/json.gp?ip=';
    if($ip!=''){
        $infoURL.=$ip;
    }
    ...
    $infoResult=wsmFnSendCurlRequest($infoURL);
        ...
        if(isset($infoResult->geoplugin_city) && isset($infoResult->geoplugin_countryCode))
		   {
			
			   $infoResult_array['city'] = $infoResult->geoplugin_city;
			   $infoResult_array['country'] = $infoResult->geoplugin_countryName;
			   $infoResult_array['countryCode'] = $infoResult->geoplugin_countryCode;
			   $infoResult_array['regionName'] = $infoResult->geoplugin_regionName;
			   $infoResult_array['lat'] = $infoResult->geoplugin_latitude;
			   $infoResult_array['lon'] = $infoResult->geoplugin_longitude;
...
function wsmFnGetFilterPostData(){
    ...
    if(isset($_REQUEST["{$prefix}FilterSubmit"]) || isset($_REQUEST["{$prefix}p"])){
        foreach($arrPostData as $key=>$value){
            $arrPostData[$key]=(isset($_REQUEST[$key])&& $_REQUEST[$key])!=''? sanitize_text_field($_REQUEST[$key]):$arrPostData[$key];
        }
    }   
    return $arrPostData;
}
...
function wsmGetVisitorIdFromCookie(){
    ...
    $strVisitor=isset($_COOKIE[$cookieId])? sanitize_text_field($_COOKIE[$cookieId]):'';
 ...
 * @return mixed
 */
function wsm_sanitize_text_or_array_field($array_or_string) {
