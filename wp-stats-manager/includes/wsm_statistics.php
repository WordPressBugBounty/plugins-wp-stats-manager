...
class wsmStatistics
{
    ...
    function wsm_showDayStats($atts, $content = "")
    {
        ...
        <li><div class="wsmCounters"><span class="wsmH2Number">' . $todayPageViews . '</span><span id="' . WSM_PREFIX . 'TodayTotalPageViews" class="wsmBoxUPDdataTIP" data-value="' . esc_attr($todayPageViews) . '"></span></div><label>' . __('Page Views', 'wp-stats-manager') . '</label></li>
        <li><div class="wsmCounters"><span class="wsmH2Number">' . $todayVisitors . '</span><span id="' . WSM_PREFIX . 'TodayTotalVisitors" class="wsmBoxUPDdataTIP" data-value="' . esc_attr($todayVisitors) . '"></span></div><label>' . __('Visitors', 'wp-stats-manager') . '</label></li>
        <li><div class="wsmCounters"><span class="wsmH2Number">' . $firstTimeVisitors . '</span><span id="' . WSM_PREFIX . 'TodayTotalFirstVisitors" class="wsmBoxUPDdataTIP"  data-value="' . esc_attr($firstTimeVisitors) . '"></span></div><label>' . __('First Time Visitors', 'wp-stats-manager') . '</label></li>
        <li style="display:none"><div class="wsmCounters"><span class="wsmH2Number">' . esc_html($onlineVisitors) . '</span><span id="' . WSM_PREFIX . 'TodayOnlineUsers" class="wsmBoxUPDdataTIP"  data-value="' . esc_attr($onlineVisitors) . '"></span></div><label><img src="' . WSM_URL . '/images/online-red.gif" alt="Online"/>&nbsp;' . __('Users Online', 'wp-stats-manager') . '</label></li>
    ...
    function wsm_showGenStats($atts, $content = "")
    {
		...
		<li class="greenTxt" style="display:none"><label>' . __('User Online', 'wp-stats-manager') . '</label><div class="wsmCounters wsmPushRight"><span id="' . WSM_PREFIX . 'GenUserOnline" class="wsmBoxUPDdataTIP" data-value="' . $onlineVisitors . '"></span><span class="wsmH2Number">' . $onlineVisitors . '</span></div></li>
...
<div class="wsmForecastHeader">' . esc_html(__($atts['title'], 'wp-stats-manager')) . '</div>
    ...
    function wsm_showStatFilterBox($atts, $content = "")
    {
        ...
        $html = '<div id="' . WSM_PREFIX . '_' . sanitize_key($atts['id']) . '" class="wsmFilterWrapper">';
 ...
 $atts = shortcode_atts( array(		
        ...
        $id = sanitize_key('_dayStatBox');
        $title = esc_html(__('Daily Stats', 'wp-stats-manager'));
        $type = sanitize_key($arrPostData['filterType']);
        $condition = sanitize_key($arrPostData['filterWay']);
        $from = sanitize_text_field($arrPostData[$condition]['from']);
        $to = sanitize_text_field($arrPostData[$condition]['to']);
        ...
        $html = '<div id="' . WSM_PREFIX . sanitize_key('_dayStatBox') . '">';
        switch ($atts['type']) {
            case 'Hourly':
 ...
 $atts = shortcode_atts( array(		
        ...
        $title = esc_html(__('Show Stats', 'wp-stats-manager'));
        $id = sanitize_key('TableTrafficStats');
        $type = sanitize_key($arrPostData['filterType']);
        $condition = sanitize_key($arrPostData['filterWay']);
        $from = sanitize_text_field($arrPostData[$condition]['from']);
        $to = sanitize_text_field($arrPostData[$condition]['to']);
 ...
 $atts = shortcode_atts( array(		
        ...
        $id = sanitize_key('_refferStatsBox');
        $title = esc_html(__('Daily Stats', 'wp-stats-manager'));
        $type = sanitize_key($arrPostData['filterType']);
        $condition = sanitize_key($arrPostData['filterWay']);
        $from = sanitize_text_field($arrPostData[$condition]['from']);
        ...
        $html = '<div id="' . WSM_PREFIX . sanitize_key('_refferStatsBox') . '" class="wsmTableContainer">';
        /*echo $atts['type']."<br/>";
        ...
        $html .= '<tr><th>' . __('Summary', 'wp-stats-manager') . '</th><th>' . __('Stats Summary', 'wp-stats-manager') . '</th></tr>';
 ...
 $atts = shortcode_atts( array(		
        ...
        $html = '<div id="' . WSM_PREFIX . sanitize_key('_topRefferStatsList') . '">';
        switch ($atts['type']) {
            case 'Hourly':
                if ($atts['condition'] == 'Normal' || $atts['condition'] == 'Range') {
                    $arrAtts['from'] = $atts['from'];
 ...
 $atts = shortcode_atts( array(		
        ...
        $condition = sanitize_text_field($arrPostData['filterWay']);
        $from = sanitize_text_field($arrPostData[$condition]['from']);
        $to = sanitize_text_field($arrPostData[$condition]['to']);
        $first = sanitize_text_field($arrPostData[$condition]['first']);
        $second = sanitize_text_field($arrPostData[$condition]['second']);
        ...
        $html = '<div id="' . WSM_PREFIX . sanitize_key('_topRefferStatsList') . '" class="wsmTableContainer">';
        $lastFrom = $lastTo = '';
        $format = 'd F Y';
        $arrCompare = array();
        ...
        if (count($arrReferrerList['data']) > 0) {
            ...
            $html .= '<table class="wsmTableStriped">';
            $html .= '<tr><th>' . __('Rank', 'wp-stats-manager') . '</th><th>' . __('Referrer url', 'wp-stats-manager') . '</th><th>&nbsp;</th></tr>';
            foreach ($arrReferrerList['data'] as $referrer => $referrers) {
                ...
                $html .= '<td><div  class="wsmTooltip">' . $arrow . '&nbsp;' . $counter . $toolTip . '</div></td><td><a title="click to open" target="_blank" rel="nofollow" href="' . $kTitle . '">' . $kTitle . '</a></td><td width="15"><a href="#" class="" data-referral_prg=\'' . (json_encode($jsParamAarray)) . '\'></a></td><td></td>';
                $html .= '</tr>';
                $counter++;
            }
 ...
 $atts = shortcode_atts( array(		
		...
		if(count($arrSearchEngineStatList) > 0)
		{
			...
			foreach( $arrSearchEngineStatList as $key => $arrSearchEngineStat ){
					...
					$user_per = sprintf('%.2f', intval($number));
 ...
 $atts = shortcode_atts( array(		
        ...
        if (is_array($arrVisitorsInfo) && count($arrVisitorsInfo)) {
            ...
            foreach ($arrVisitorsInfo as $key => $data) {
                $unique_id = rand();
                $contentData .= '<div class="vistor_panel_data wsmTableContainer" id="vistor_panel_' . esc_attr($unique_id) . '">';
                ...
                $contentData .= '<tr><th>' . __('Rank', 'wp-stats-manager') . '</th><th>' . esc_html($title) . '</th><th>&nbsp;</th><th>' . __('References', 'wp-stats-manager') . '</th></tr>';
                ...
                if (count($data) > 0) {
                    foreach ($data as $data_key => $row) {
                        ...
                        if ($atts['condition'] == 'Normal') {
                            ...
                            $format = 'd F Y';
                            $toolTip = '<span class="wsmTooltipText">' . esc_html($row['name']) . '<font class="wsmColorYellow">' . wsmGetDateByTimeStamp($format, strtotime($arrAtts['from'])) . '&nbsp;/&nbsp;' . wsmGetDateByTimeStamp($format, strtotime($lastFrom)) . '</font><font>' . __('Visitors Performance', 'wp-stats-manager') . ':&nbsp;<b class="' . $class2 . '">' . $percent . '%</b></font></span>';
                        }
                        ...
                        $contentData .= '<tr id="wsmRowParent_' . esc_attr($counter) . '" class="wsmReferralRow"><td><div class="wsmTooltip">' . $arrow . '&nbsp;' . $counter . $toolTip . '</div></td><td>' . $row['name'] . '</td><td></td><td>' . number_format($row['total']) . $compareValue . '</td></tr>';
                ...
                $header .= $seperator . '<a class="' . esc_attr($link_class) . '" 1=\'' . json_encode($visitor_graph) . '\' href="#vistor_panel_' . esc_attr($unique_id) . '">' . esc_html($key) . '</a>';
                $link_class = '';
                $seperator = ' | ';
            }
        }
        ...
        if (current_user_can('edit_others_pages')) {
            ...
            echo '<div class="stats_submenu">' . wsmInitPlugin::wsm_strip_tags($header) . '</div>' . wsmInitPlugin::wsm_strip_tags($contentData);
        } else {
            return  wsmInitPlugin::wsm_strip_tags($contentData);
        }
    }
 ...
 $atts = shortcode_atts( array(		
		...
		if( is_array($arrVisitorsInfo) && count($arrVisitorsInfo) ){
			...
			foreach( $finalArrVisitorsInfo as $key => $data ){
				...
				echo '<div id="vistor_panel_'.esc_attr($unique_id).'" class="each_visior_info_graph" data-graph=\''.json_encode($visitor_graph).'\'>'. $header.'</div>';				
			}
		}
    ...
    function wsm_showStatKeywords($atts)
    {
        ...
        if (count($totalSearchKeywords['data']) > 0) {
            ...
            foreach ($totalSearchKeywords['data'] as $searchKeyword) {
                ...
                $html .= sprintf('<td><div>%s<div class="pull-right">%s - <span>%s</span></div></div><a href="%s" target="_blank">%s</a></td>', esc_html($keyword), date('Y/m/d', strtotime($searchKeyword['serverTime'])), esc_html($searchKeyword['ipAddress']), esc_url($searchKeyword['protocol'] . $searchKeyword['url']), esc_html($searchKeyword['url']));
                $html .= '</tr>';
                $counter++;
            }
 ...
 $atts = shortcode_atts( array(		
        ...
        $id = sanitize_key('__geoLocation');
        $title = esc_html(__('Geo Location', 'wp-stats-manager'));
        $type = sanitize_text_field($arrPostData['filterType']);
        $condition = sanitize_text_field($arrPostData['filterWay']);
        $from = sanitize_text_field($arrPostData[$condition]['from']);
        $to = sanitize_text_field($arrPostData[$condition]['to']);
        ...
        if (count($totalGeoLocationDetails) > 0) {
            ...
            foreach ($totalGeoLocationDetails as $key => $row) {
                ...
                $contentData .= '<tr id="wsmRowParent_' . $counter . '" class="wsmReferralRow">
					...
					<td><img src="' . WSM_URL . 'images/ICO_1px.gif" class="flag flag-' . strtolower($row['alpha2Code']) . '" alt="' . $row['name'] . '" title="' . $row['name'] . '"/>&nbsp;'  . $row['name'] . '
					</td>
 ...
 $atts = shortcode_atts( array(		
        ...
        $atts = shortcode_atts(array(
            ...
            'limit' => 50,
            'condition' => sanitize_text_field($arrPostData['filterWay']),
            'from' => sanitize_text_field($arrPostData[$arrPostData['filterWay']]['from']),
            'to' =>  sanitize_text_field($arrPostData[$arrPostData['filterWay']]['to']),
        ...
        if (isset($_REQUEST['search']) && $_REQUEST['search']) {
            $arrAtts['search'] = sanitize_text_field($_REQUEST['search']);
        }
        if (current_user_can('edit_others_pages')) {
            $admin_url = admin_url('admin.php?page=' . WSM_PREFIX . '_content');
            if (isset($_GET['subPage']) && $_GET['subPage'] == 'byTitle') {
                $admin_url .= '&subPage=' . sanitize_text_field($_GET['subPage']);
            }
        ...
        if (is_array($urlStatsResult) && isset($urlStatsResult['data']) && count($urlStatsResult['data']) > 0) {
            ...
            $html .= '<input type="text" name="search" value="' . esc_attr($search) . '" />';
            $html .= '<span class="dashicons dashicons-search"></span>';
            $html .= '</div>';
            $html .= '<table class="wsmTableStriped">';
            $html .= '<tr><th>' . __('Rank', 'wp-stats-manager') . '</th>
						<th>' . $urlTitle . '</th>
						<th class="width_100 align_center"><a href="' . esc_url($order_query) . '">' . __('Hits', 'wp-stats-manager') . ' <span class="dashicons ' . esc_attr($order_class) . '"></span></a></th>
            ...
            foreach ($urlStatsResult['data'] as $row) {
                ...
                if ($condition == 'Normal') {
                    ...
                    $toolTip = '<span class="wsmTooltipText">' . esc_html($name) . '<font class="wsmColorYellow">' . wsmGetDateByTimeStamp($format, strtotime($startFrom)) . '&nbsp;/&nbsp;' . wsmGetDateByTimeStamp($format, strtotime($lastFrom)) . '</font><font>' . __('Visitors Performance', 'wp-stats-manager') . ':&nbsp;<b class="' . esc_attr($class2) . '">' . intval($percent) . '%</b></font></span>';
                }
 ...
 $atts = shortcode_atts( array(		
    ...
    function wsm_showIPExclustion($atts)
    {
        ...
        $html .= '<input type="button" id="save_ipadress" class="save_ipadress button button-primary" value="' . __('Save IP Address', 'wp-stats-manager') . '" />';
        $html .= '</div>';
        ...
        if ($ipAddress && is_array($ipAddress)) {
            foreach ($ipAddress as $address => $status) {
                $html .= '<tr id="row_' . esc_attr($count) . '">';
                $html .= '<td>' . esc_html($count) . '</td>';
                $html .= '<td>' . esc_html($address) . '</td>';
                $html .= '<td><label class="switch"><input data-ipaddress="' . esc_attr($address) . '" type="checkbox" ' . ($status ? 'checked' : '') . '><div class="slider round"></div></label></td>';
                $html .= '<td><a href="#" data-row="' . esc_attr($count) . '" data-ipaddress="' . esc_attr($address) . '" class="deleteIP button button-secondary">' . __('Delete', 'wp-stats-manager') . '</a></td>';
                $html .= '</tr>';
                $count++;
            }
        } else {
